...
//        $query = $this->db->query("SELECT COUNT(*) as lot_count FROM lots_tbl WHERE status = 1 AND company_id = ".$this->company_id);
    ...
    public function get_lots_by_tracts($tid) {
        $query = $this->db->query("SELECT COUNT(*) as lot_count, tract_id FROM lots_tbl WHERE tract_id = $tid AND status = 1 AND company_id = ".$this->company_id);
        return $query->row();
    }
    ...
    public function get_diff_days($lot_id) {
        $query = $this->db->query("SELECT DATEDIFF(MAX(completion_date), MIN(created_date)) as diff_days_count FROM defect_tbl WHERE status = 1 AND is_completed = 1 AND company_id = ".$this->company_id." AND lot_id =". $lot_id);
        return $query->row();
    }
    ...
    public function get_diff_days_tract($lot_id, $tract_id) {
        $query = $this->db->query("SELECT DATEDIFF(MAX(completion_date), MIN(created_date)) as diff_days_count FROM defect_tbl WHERE status = 1 AND is_completed = 1 AND company_id = ".$this->company_id." AND tract_id = $tract_id AND lot_id =". $lot_id);
        return $query->row();
    }
    ...
    public function get_diff_days_partner($lot_id, $partner_id) {
        $pids = '"'.$partner_id.'"';
        $partner_ids = " trade_partner LIKE '%".$pids."%' ";
        $query = $this->db->query("SELECT DATEDIFF(MAX(completion_date), MIN(created_date)) as diff_days_count FROM defect_tbl WHERE status = 1 AND is_completed = 1 AND company_id = ".$this->company_id." AND $partner_ids AND lot_id =". $lot_id);
        return $query->row();
    }
    ...
    public function get_total_defect_per_lot_partner($start_date, $end_date, $partner_id) {
        $pids = '"'.$partner_id.'"';
        $partner_ids = " trade_partner LIKE '%".$pids."%' ";
        $query = $this->db->query("SELECT COUNT(*) as partner_count, lot_id FROM defect_tbl WHERE status = 1 AND company_id = 4 AND DATE(created_date) BETWEEN '".$start_date."' AND '".$end_date."' AND $partner_ids GROUP BY lot_id");
        return $query->result();
    }
    ...
    public function get_total_days_complation_defect_per_lot_partner($partner_id) {
        $pids = '"'.$partner_id.'"';
        $partner_ids = " trade_partner LIKE '%".$pids."%' ";
        $query = $this->db->query("SELECT COUNT(*) as cnt, lot_id, DATEDIFF(MAX(completion_date), MIN(created_date)) as total_days FROM defect_tbl WHERE status = 1 AND company_id = $this->company_id AND $partner_ids AND is_completed = 1 AND lot_id NOT IN(SELECT lot_id FROM defect_tbl WHERE is_completed != 1 AND status = 1 AND company_id = $this->company_id AND $partner_ids) GROUP BY lot_id ORDER BY completion_date DESC LIMIT 20");
        return $query->result();
    }
...
//    }
    ...
    public function get_min_max_complation_date($partner_id, $lot_id){
        $pids = '"'.$partner_id.'"';
        $partner_ids = " trade_partner LIKE '%".$pids."%' AND";
        return $this->db->query("SELECT DATEDIFF(MAX(completion_date), MIN(created_date)) as diff_days_count FROM defect_tbl WHERE $partner_ids status = 1 AND is_completed = 1 AND company_id = ".$this->company_id." AND lot_id =". $lot_id)->row();
    }
