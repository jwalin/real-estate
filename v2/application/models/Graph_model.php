...
//        $query = $this->db->query("SELECT COUNT(*) as lot_count FROM lots_tbl WHERE status = 1 AND company_id = ".$this->company_id);
    ...
    public function get_total_defect_per_lot($start_date, $end_date, $tract_id) {
        $query = $this->db->query("SELECT COUNT(*) as tract_count, MAX(created_date) as cd, lot_id FROM defect_tbl WHERE status = 1 AND company_id = $this->company_id AND DATE(created_date) BETWEEN '".$start_date."' AND '".$end_date."' AND tract_id = $tract_id GROUP BY lot_id ORDER BY cd DESC");
        return $query->result();
    }
    
    public function get_total_days_complation_defect_per_lot($tract_id) {
        $query = $this->db->query("SELECT COUNT(*) as cnt, MAX(completion_date) as cd, lot_id, DATEDIFF(MAX(completion_date), MIN(created_date)) as total_days, getWorkingday(DATE_FORMAT(MIN(created_date),'%Y-%c-%d'),
		MAX(completion_date),'weekend_days') as weekend_days FROM defect_tbl WHERE status = 1 AND company_id = $this->company_id AND tract_id = $tract_id AND is_completed = 1 AND lot_id NOT IN(SELECT lot_id FROM defect_tbl WHERE is_completed != 1 AND status = 1 AND company_id = $this->company_id AND tract_id = $tract_id) GROUP BY lot_id ORDER BY cd DESC LIMIT 20");
        return $query->result();
    }
    ...
    public function get_total_defect_per_lot_partner($start_date, $end_date, $partner_id) {
        $pids = '"'.$partner_id.'"';
        $partner_ids = " trade_partner LIKE '%".$pids."%' ";
        $query = $this->db->query("SELECT COUNT(*) as partner_count, lot_id, MAX(created_date) as cd FROM defect_tbl WHERE status = 1 AND company_id = ".$this->company_id."  AND  DATE(created_date) BETWEEN '".$start_date."' AND '".$end_date."' AND $partner_ids GROUP BY lot_id ORDER BY cd DESC");
        return $query->result();
    }
