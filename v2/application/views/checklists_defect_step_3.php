            ...
            <div class="card mb-0">
                <div class="card-body p-20 text-center">
                    ...
                    <div class="col-sm-6 pull-left">
                        <form id="formid" action="<?php echo base_url('checklists_defect/insert_step_3'); ?>" method="post" enctype="multipart/form-data">
                            <?php 
                            ...
                            <input type="hidden" name="locationid" id="locationid" value="<?php echo @$this->uri->segment(6); ?>">
                            ...
                            <fieldset>
                                ...
                                <div class="col-sm-12 clearfix">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="location" name="location">
											...
											<option value="<?php echo $data_defect_location->id; ?>"><?php echo $data_defect_location->defect_location; ?></option>
                                        </select>
                                    </div>
                                        <div class="form-group">
                                            <select class="form-control" name="trade_category[]" id="cat" required="" onchange="category_to_defect(this.value)">
                                                ...
                                                foreach ($data_cat as $row_data_category) { ?>
                                                    <option value="<?php echo $row_data_category->id; ?>" <?php if($row_data_category->id == base64_decode(base64_decode(@$this->uri->segment(7)))){ echo 'selected'; } ?>><?php echo $row_data_category->category_name; ?></option>
                                                <?php } } ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <select class="form-control" id="defect_type" name="defect_type" required="" onchange="select_defect_type(this.value)">
                                        </select>
                                    </div>
                                    <div class="form-group dt_custom" style="display: none;">
                                        <input type="text" class="form-control defect_type_input" name="defect_type_input" placeholder="Enter Defect Type">
                                    </div>
                                </div>
                                ...
                                <input type="submit" name="submit" class="btn btn-primary mt-3 mr-2" value="Start Next Defect" />
                                <input type="submit" name="submit_trade_categories" class="btn btn-primary mt-3 mr-2" value="Trade Categories" />
                                <input type="submit" name="submit_location_categories" class="btn btn-primary mt-3" value="Location-Categories" />
                            </fieldset>
                        </form>
                    </div>
                    <div class="col-sm-3 pull-left">&nbsp;</div>
...
//        return false;
        ...
        $.ajax({
            url: '<?= base_url() . 'checklists_defect/ajax_trade_category_partner/' ?>',
            data: {tract_id: tract_id, locationid: locationid, total: total},
            type: 'post',
...
var loc_ids = '<?php echo @$data_defect_location->id; ?>';
var tract_id = $('#tract_id').val();
category_to_defect(cat, tract_id, loc_ids);
function category_to_defect(ele, tract_id, loc_ids){
    var loc_ids = $('#location').val();
    var tract_id = $('#tract_id').val();
    console.log(ele);
    console.log(loc_ids);
    $.ajax({
        url: '<?= base_url() . 'home/ajax_category_loc_to_defect_type/' ?>',
        data: {cat_id: ele, tract_id: tract_id, loc_ids: loc_ids},
        type: 'post',
        success: function (data) {
			console.log(data);
            var datas = JSON.parse(data);
            $('#defect_type').html(datas.defect);
            $('#defect_type').val(<?php echo base64_decode(base64_decode($this->uri->segment(8))); ?>);
...
function select_defect_type(ele){
    $('.defect_type_input').val('');
    $('.dt_custom').hide();
    $('.defect_type_input').attr('required', false);
    if(ele == "Other"){
        $('.dt_custom').show();
        $('.defect_type_input').attr('required', true);
    }
}
</script>